...
var import_node_path, import_node_os, CSS_LANGS_RE, knownJsSrcRE, importQueryRE, isImportRequest, isJSRequest, isCSSRequest, isWindows;
...
var init_transformRequest = __esm({
  "src/node/server/transformRequest.ts"() {
    "use strict";
  }
});
  ...
  return {
    ...
    async resolveId(id, importer) {
      ...
      if (id[0] === "/" && !id.startsWith(root)) {
        const fsPath = import_path4.default.resolve(root, id.slice(1));
        return { id: fsPath };
      }
      if (id.startsWith(".")) {
        const basedir = importer ? import_path4.default.dirname(importer) : process.cwd();
        const fsPath = import_path4.default.join(basedir, id);
        return { id: fsPath };
      }
      if (import_path4.default.isAbsolute(id)) {
        return { id };
      }
      if (bareImportRE.test(id)) {
        let prefix = import_path4.default.resolve(root, "node_modules", id);
        const module2 = require(prefix + "/package.json").module;
        const fsPath = import_path4.default.join(prefix, module2);
        return { id: fsPath };
      }
      return null;
    }
...
var import_path4, bareImportRE;
var init_rosolve = __esm({
  "src/node/plugin/rosolve.ts"() {
    "use strict";
...
function buildPlugin() {
  return {
    ...
    async load(id) {
      try {
        const code = await (0, import_promises3.readFile)(id, "utf-8");
        return code;
      } catch (e) {
        console.log(e);
        return null;
...
async function createPluginContainer(config) {
  ...
  const container = {
    async resolveId(rawId, importer = (0, import_path5.join)(config.root, "index.html")) {
...
var import_path5;
var init_pluginContainer = __esm({
  "src/node/server/pluginContainer.ts"() {
    "use strict";
    init_plugin();
...
var findAllBrackets = (v) => {
  ...
  while (angledMatch = ANGLED_BRACKET_RE_GLOBAL.exec(v)) {
    res.push(parse4(angledMatch));
  }
  let squareMatch;
  while (squareMatch = SQUARE_BRACKET_RE_GLOBAL.exec(v)) {
    res.push(parse4(squareMatch));
  }
  return res;
};
